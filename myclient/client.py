...
BASE_URL = "http://sc22zy.pythonanywhere.com/rating/"  
...
def login():
    ...
    user_input = input("Enter login command (e.g., login sc22zy.pythonanywhere.com): ").strip()

    parts = user_input.split()
    if len(parts) == 2 and parts[0] == "login":
        url = parts[1]
    else:
        url = input("Enter service URL (e.g., sc22zy.pythonanywhere.com): ").strip()
...
def list_module_instances():
    ...
    try:
        ...
        for instance in data:
            professors = ", ".join([f"{prof['id']}, {prof['name']}" for prof in instance["professors"]])
